...
async function importStocks() {
    ...
    const symbolsArray = JSON.parse(symbols) as string[];
    ...
    groupedSymbols.forEach(async (symbolsArray: string[]) => {
        ...
        try {
            console.log("inserting 200 stocks...");
            await db.insert(stocks).values(companyInfoArray).onConflictDoNothing();
            console.log('200 stocks inserted successfully!');
        } catch (error) {
